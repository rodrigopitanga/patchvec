...
release:
	...
	FILES="setup.py README.md Dockerfile docker-compose.yml $(PKG_INTERNAL)/main.py CHANGELOG.md"; \
	revert_changes() { \
	  echo "Reverting version bumps and changelog..."; \
	  git checkout -- $$FILES || true; \
	...
	HEADER="## $(VERSION) — $$DATE\n\n### Commits"; \
	{ echo "$$HEADER"; echo "$$COMMITS" | sed 's/^/- /'; echo "\n---"; cat CHANGELOG.md 2>/dev/null || true; } > CHANGELOG.md.new; \
	mv CHANGELOG.md.new CHANGELOG.md; \
	# Stage changes so tests/build failures can easily revert
	git add $$FILES; \
	# 3) Tests
	echo "Running tests..."; \

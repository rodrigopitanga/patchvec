...
"""
CollectionDB — impl0: single persistent connection, check_same_thread=False.

A threading.Lock serialises writes; reads (WAL) need no lock.
"""
...
class CollectionDB:
    """Per-collection SQLite metadata store (impl0).

    Single persistent connection with check_same_thread=False.
    An internal threading.Lock serialises write operations.
    ...
    def open(self, path: Path) -> None:
        ...
        parent = path.parent
        if parent.exists():
            if (parent / "catalog.json").exists() or \
               (parent / "meta.json").exists():

...
"""
CollectionDB — impl1: thread-local connections.

Each thread opens its own sqlite3.Connection lazily.
A class-level threading.Lock serialises write operations across threads.
WAL mode allows concurrent reads without a lock.
"""
...
class CollectionDB:
    """Per-collection SQLite metadata store (impl1).

    Thread-local connections: each thread has its own sqlite3.Connection,
    opened lazily on first use. A class-level Lock serialises writes.
    WAL mode allows concurrent reads without any lock.
    """
    ...
    def _conn(self) -> sqlite3.Connection | None:
        """Return current thread connection (for test introspection)."""
        conn = getattr(self._local, "conn", None)
        if conn is None and self._path is not None:
            conn = self._get_conn()
        return conn
    ...
    def open(self, path: Path) -> None:
        ...
        self._path = path
        # Open the calling-thread connection and run migrations
        self._apply_migrations()

    def close(self) -> None:
        """Close the current thread connection."""
        conn = getattr(self._local, "conn", None)
        if conn is not None:
            conn.close()
    ...
    # ------------------------------------------------------------------
    # Write operations — serialised by _write_lock
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Read operations — no lock needed (WAL)
    # ------------------------------------------------------------------
    ...
    def get_meta_batch(self, rids: list[str]) -> dict[str, dict[str, Any]]:
        ...
        Called OUTSIDE collection_lock — WAL reads are concurrent.
